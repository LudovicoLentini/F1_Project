...
m.1<- glm(as.factor(dati.positionText) ~. ,data = x_train,family=binomial(link="logit"))
c.1<-x_test
pred.1<-predict(m.1,c.1)
predd.1<- data.frame(y_test,pred.1)
rmse_lrl<-rmse(as.numeric(predd.1$y_test[is.na(predd.1$pred.1)==FALSE]),as.numeric(predd.1$pred.1[is.na(predd.1$pred.1)==FALSE]))
rmse_lrl
...
m.2<- glm(as.factor(dati.positionText) ~. ,data = x_train,family=binomial(link="probit"))
c.2<-x_test
pred.2<-predict(m.2,c.2)
predd.2<- data.frame(y_test,pred.2)
rmse_lrp<-rmse(as.numeric(predd.2$y_test[is.na(predd.2$pred.2)==FALSE]),as.numeric(predd.2$pred.2[is.na(predd.2$pred.2)==FALSE]))
rmse_lrp
...
library(caret)
rf <- randomForest(as.factor(dati.positionText) ~. , data = x_train, proximity = TRUE, na.action=na.exclude)
print(rf)
...
results <- append(results, rmse_rf)
```
